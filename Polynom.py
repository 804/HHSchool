...
def add_operand_in_stack(operand, reverse_polish, operand_stack):
    ...
    :param reverse_polish: output in the form of reverse Polish notation
    :type reverse_polish: <type 'list'>
    ...
    while position < len(expression):
        if (position != 0) and (expression[position - 1] == ')'):
            if expression[position] in (numbers + ['x', '(']):
                add_operand_in_stack('*', reverse_polish, operand_stack)
        ...
        if expression[position] == '(':
            ...
            if limits:
                ...
                reverse_polish.append(expression[position:])
                position = len(expression)
    while operand_stack:
        reverse_polish.append(operand_stack.pop())

    position = 0
    reverse_polish = [string_monomial_to_dictionary(string) if string not in ('+', '-', '*') else string for string in
                      reverse_polish]

    while position < len(reverse_polish):
        if reverse_polish[position] == '+':
            reverse_polish[position - 2:position + 1] = [
                addition_polinomials(reverse_polish[position - 2], reverse_polish[position - 1])]
            position -= 1
            continue
        elif reverse_polish[position] == '-':
            reverse_polish[position - 2:position + 1] = [
                subtraction_polinomials(reverse_polish[position - 2], reverse_polish[position - 1])]
            position -= 1
            continue
        elif reverse_polish[position] == '*':
            reverse_polish[position - 2:position + 1] = [
                multiple_polinomials(reverse_polish[position - 2], reverse_polish[position - 1])]
            position -= 1
            continue
        position += 1
